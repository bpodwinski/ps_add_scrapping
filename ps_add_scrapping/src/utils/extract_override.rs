...
pub fn extract_override(html_content: &str) -> Option<String> {
    let document = Html::parse_document(html_content);
    let selector = Selector::parse("div").unwrap();

    // Find the div containing the specific text
    let target_div = document.select(&selector).find(|element| {
        element
            .text()
            .any(|text| text.contains("Contient des surcharges"))
    });
    ...
    if let Some(div) = target_div {
        ...
        println!("Aucune div contenant 'Contient des surcharges' trouvée.");
    }
